...
class OnlinePlanner:
    ...
    def get_goal(self, goal):
        if self.svc.there_is_map:
            ...
            # Extract goal position (x, y) from the message
            goal_position = goal.pose.position
            self.goal = np.array([goal_position.x, goal_position.y])

            # Print the goal position
            rospy.loginfo("Goal: [%f, %f]", self.goal[0],self.goal[1])
    ...
    def plan(self):
        ...
        attempt = 1 # to limit the number of attempts to replan before considered failed

        rospy.loginfo("Compute new path")

        while(self.path == []) and attempt <= 5:
            max_time = 3.0
            # TODO: plan a path from self.current_pose to self.goal
            self.path = compute_path(self.current_pose[:2], self.goal, self.svc, self.bounds, max_time=max_time)
        
            # TODO: If planning fails, consider increasing the planning time, retry the planning a few times, etc.
            if len(self.path) == 0:
                rospy.logerr("Path not found! Retrying...")
                max_time += 1.0
                attempt += 1
                
            elif len(self.path) > 0: # if planning succeed
        ...
        if self.path == []:
            rospy.logerr("Path not found after 5 attempts! Choose another goal!")
    ...
    def controller(self, event):
        ...
        if len(self.path) > 0:
            ...
            if np.linalg.norm(self.current_pose[:2] - self.path[0]) < 0.1: #( 0.1 Tolerance) Calculate the distance b/w current position and waypoint. 
                rospy.loginfo("Waypoint Reached: [%f, %f]", self.path[0][0], self.path[0][1])
                del self.path[0] # Move to next waypoint
